...
async function waitForRedisReady(): Promise<void> {
  ...
  if (redis.status !== 'ready') {
    logger.warn('[Orchestrator] Redis not ready after 3s — continuing without guaranteed state persistence');
  }
}
 ...
 */
async function startAccountBlockCooldown(): Promise<void> {
  ...
  logger.warn('');
  logger.warn('═══════════════════════════════════════════════════════');
  logger.warn(`  [${ts()}] [Orchestrator] ACCOUNT BLOCK COOLDOWN — ${ACCOUNT_BLOCK_COOLDOWN_MS / 60000} min`);
  logger.warn(`  [${ts()}] [Orchestrator] Next session launches at: ${endTime}`);
  logger.warn(`  [${ts()}] [Orchestrator] Chrome is fully shut down`);
  logger.warn('═══════════════════════════════════════════════════════');
}
...
async function runSessionWithRetry(
  ...
  try {
    ...
    if (err instanceof AccountBlockedError) {
      logger.warn(`[${ts()}] [Orchestrator] ${phase} — account blocked (429001), skipping retry`);
      throw err;
    }
  ...
  try {
    ...
    if (err instanceof AccountBlockedError) {
      logger.warn(`[${ts()}] [Orchestrator] ${phase} — account blocked (429001) on retry`);
      throw err;
    }
...
export async function startOrchestrationLoop(callbacks: OrchestratorCallbacks): Promise<void> {
  ...
  logger.info(`[${ts()}] [Orchestrator] Redis status: ${redis.status}`);
  ...
  while (true) {
    ...
    try {
      ...
      if (err instanceof AccountBlockedError) {
        ...
        logger.warn(`[${ts()}] [Orchestrator] Account blocked — force-rotating and entering 30-min cooldown`);

        const nextEmail = await forceRotateOnBlock(blockedEmail);

        // Notify owner via personal DM
        notifyOwnerAccountBlocked(blockedEmail, nextEmail, ACCOUNT_BLOCK_COOLDOWN_MS / 60000).catch((e) => {
      ...
      if (err instanceof OtpTimeoutError) {
        ...
        logger.warn(`[${ts()}] [Orchestrator] OTP timeout — force-rotating and entering 30-min cooldown`);

        const nextEmail = await forceRotateOnBlock(timedOutEmail);

        // Notify owner via personal DM
        notifyOwnerOtpTimeout(timedOutEmail, nextEmail, ACCOUNT_BLOCK_COOLDOWN_MS / 60000).catch((e) => {
          logger.warn({ err: e.message }, '[Orchestrator] Owner DM failed — non-fatal');
        });

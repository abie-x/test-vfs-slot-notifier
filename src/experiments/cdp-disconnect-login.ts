/**
 * Experiment — CDP disconnect before Turnstile + auto Sign In click
 *
 * Proven: Turnstile passes when CDP is disconnected before it renders.
 * This experiment adds: auto Sign In click after Turnstile passes.
 *
 * Flow:
 *   1. Launch Chrome with --remote-debugging-port
 *   2. Connect CDP → navigate to login → fill email + password
 *   3. Disconnect CDP → Turnstile renders and passes
 *   4. Reconnect CDP → wait for Sign In button to enable → click it
 *   5. You enter OTP manually
 *   6. Press ENTER → polling starts
 */
...
async function main(): Promise<void> {
  logger.info('Experiment — CDP disconnect login + auto Sign In');
  logger.info('═══════════════════════════════════════════════════════');

 ...
 */
export async function waitForOtp(maxWaitMs = 2 * 60 * 1000): Promise<string | null> {
  logger.info('Waiting for OTP email from VFS...');
  ...
  while (Date.now() < deadline) {
    try {
      ...
      for (const msg of messages) {
        ...
        if (Date.now() - internalDate > MAX_AGE_MS) continue;
        ...
        if (match) {
          logger.info({ otp: match[1] }, '✓ OTP found in Gmail');
          return match[1];
        }
      }
    } catch (err) {
      logger.warn({ err }, 'Gmail API error — retrying');
    }

    const remaining = Math.round((deadline - Date.now()) / 1000);
    logger.info(`Waiting for OTP email... (${remaining}s remaining)`);

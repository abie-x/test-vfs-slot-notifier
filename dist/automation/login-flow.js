...
async function retryLoginWithReload(email, password) {
    ...
    const { clicked, client: newClient } = await (0, cdp_helpers_1.checkButtonWithRetry)(BUTTON_SELECTOR, 'Sign In', 5, 60, 7);
    if (!clicked) {
        logger_1.logger.error('Sign In still failed after reload — manual intervention required');
        process.exit(1);
    }
    return newClient;
}
...
async function retryOTPSubmitWithReload(otp, email, password) {
    ...
    if (screen === 'login') {
        ...
        if (!freshOtp) {
            logger_1.logger.error('[OTP retry] Could not fetch fresh OTP — manual intervention required');
            process.exit(1);
        }
